...
- Ciphers support Scandinavian alphabets (Æ/Ø/Å, Danish/Norwegian/Swedish variants) via a per-cipher alphabet dropdown reading `CAESAR_ALPHABETS` — Caesar, A1Z26, Affine, Playfair, Polybius, Bacon, Morse, Beaufort, Autokey, Keyword Substitution, and the brute-force helper all follow this pattern. The transposition ciphers (Columnar, Scytale) are position-based and alphabet-agnostic, so Scandinavian characters pass through untouched. Younger Futhark is a lossy 16-rune transliteration that maps Æ/Ø/Å (and Ä/Ö) onto their nearest runes. Some (Playfair/Polybius) switch grid dimensions for the 29-letter alphabets; Affine's modulus becomes 29 (prime, so every multiplier is valid — the `a` dropdown is rebuilt from `affineCoprimes(m)`). Retired registry ids (e.g. `scandicaesar`, merged into `caesar`) are mapped forward in `migrateCipherId()` in `state.js`.
